...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .card {
      position: relative;
      background: linear-gradient(180deg, #131828, #101422);
    ...
    .cover { height: 150px; background: linear-gradient(135deg, #182340, #0e1426); display:grid; place-items:center; font-size: 46px; }
    ...
    .modal .cover { height: 100%; border-radius: 12px; }
    .modal h2 { margin: 0; }
...
<body>
  ...
  <footer>
    © <span id="year"></span> • by Mateusz Węglewski • <a class="link" href="https://github.com/mateuszweglewski" target="_blank" rel="noopener">See my other projects</a>
  </footer>
  ...
  <dialog id="dlg">
    ...
    <div class="modal">
      <div class="cover" id="dlgCover">📖</div>
      <div>
        <h2 id="dlgTitle"></h2>
  ...
  <script>
    ...
    const books = [
      {
        id: '1',
        title: 'r3dh3rr1n6',
        author: 'Mateusz Węglewski',
        year: 2024,
        tags: ['who','knows'],
        description: 'You will find out, soon',
        coverEmoji: '🐟', 
        url: 'sth.pdf',
        format: 'PDF'
      },
    ...
    function renderTags(){
      ...
      box.appendChild(make('')); // reset placeholder
    ...
    function cardFrom(book){
      ...
      $('.meta', node).innerHTML = `${book.author ? `<span>${book.author}</span>`:''}${book.year?`<span>• ${book.year}</span>`:''}`;
    ...
    function openModal(b){
      ...
      $('#dlgYear').textContent = b.year ? `Year: ${b.year}` : 'Year: —';
      $('#dlgFormat').textContent = b.format || '—';
      ...
      if(b.coverUrl){
        cover.style.backgroundImage = `url(${b.coverUrl})`;
        cover.style.backgroundSize = 'cover';
        cover.style.backgroundPosition = 'center';
        cover.textContent = '';
      } else { cover.textContent = b.coverEmoji || '📖'; cover.style.background='linear-gradient(135deg,#182340,#0e1426)'; }
